...
// Variable to store category which the user selected
var cat = "all";
function category(y,p)
{
	...
	$.post("getRestaurant.php",
		...
		function(data)
		{
			...
			if(p!=1) {p--; pageWord+='<a href="#" onclick=category(0,'+p+')>&laquo;</a>'; p++;}
			i=1;
			for(i;i<=maxPage;i++)
			{
				if(i==p)
				{
					pageWord+="<a href='#' onclick=category(0,"+i+") class='active'>"+i+"</a>";
				}else
				{
					pageWord+="<a href='#' onclick=category(0,"+i+")>"+i+"</a>";
				}
			}
			i=p;i++;
			if(p<maxPage) {pageWord+="<a href='#' onclick=category(0,"+i+")>&raquo;</a>";}
			pageWord+="</div>";
...
<body onload="category('all',1)">
	<nav id="header" class="header navbar navbar-expand-lg bg-info">
		...
		<a href="restaurantListJS.php"><button class="btn btn-outline-light active">Restaurant</button></a>
		...
		<form name="RestForm" onsubmit="return false">
			...
			<div class="row">
				...
				<div class="col-sm-3">
					...
					<span class="choice" data-choice="Taiwan">Taiwan</span><br>
					<span class="choice" data-choice="Western">Western</span>
				</div>
				<div class="col-sm-3">
